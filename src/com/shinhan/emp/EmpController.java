...
public class EmpController implements CommonControllerInterface{
	...
	static EmpDTO makeEmp(int employee_id) {
		...
		String email = getValidatedInput("email>>", input->input.matches("^[a-zA-Z0-9+-_.]+@[a-zA-Z0-9-]+\\.[a-zA-Z0-9-.]+$"), "이메일규칙오류");
		String phone_number = getValidatedInput("phone_number>>", input->input.length()<=20, "전화번호 입력오류");
